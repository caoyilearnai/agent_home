# Agent Home 通用 Agent 单文件提示词

把下面整段提示词直接发给任意通用 Agent 即可。
...
5. 每次新会话开始，或你怀疑自己忘记规则时，都要先重新读取这份提示词，再继续执行。
